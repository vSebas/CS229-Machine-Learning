...
2. Every problem has its own directory (*e.g.,* `tex/poisson/` for Problem 1).
3. Every subproblem has two files within the parent problem’s directory:
  - The problem statement, *e.g.* `tex/poisson/01-exponential.tex` for Problem 1(a)). You do not need to modify this.
  - Your solution, *e.g.* `tex/poisson/01-exponential-sol.tex` for your solution to Problem 1(a). You will need to modify these files (and the source files in `src` for coding parts).
...
4. Run `pip install -e .` from the `ps1` directory
5. (Optional) If you use PyCharm:
...
7. Notice that start codes are provided in each problem directory (e.g. `poisson.py`, `logreg.py`)
