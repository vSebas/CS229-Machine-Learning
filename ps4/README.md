...
5. Notice some coding problems come with a `util.py` file. In it you have access to methods that do the following tasks:
...
6. Notice that starter codes are provided in each problem directory (e.g. `gda.py`, `posonly.py`)
...
7. Once you are done with all the code changes, run `make_zip.py` to create a `submission.zip`.
